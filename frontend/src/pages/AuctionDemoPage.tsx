...
export function AuctionDemoPage() {
  const { tokenState, isAuthenticated } = useDemoSession();
  ...
  return (
    <>
      <ApiTraceDrawer
        ...
        description="Inspect product and bid request/response payloads when you need application-level observability."
        traces={traces}
          ...
          <div className="flex flex-col justify-between gap-6 rounded-[2rem] border border-zinc-200/60 bg-white p-8 sm:p-10 lg:flex-row lg:items-end">
            ...
            <div className="w-full rounded-[1.5rem] bg-zinc-50 p-5 lg:max-w-xs">
              ...
              <p className="mt-2 truncate text-sm text-zinc-500">
                {isAuthenticated ? tokenState?.user.email || tokenState?.user.username : "Sign in to create products or bid."}
              </p>
              {!isAuthenticated ? (
                <Link
                  to="/demo/auth"
